...
public class Runner {
    ...
    private static Map<String, Object> runCode(String code, String userInput) {
        try {
            Path cpp = Files.createTempFile("Main", ".cpp");
            Path exe = cpp.getParent().resolve("a.out");
            Files.writeString(cpp, code);

            // Compile
            Map<String, Object> compileResult = runProcess(
                    List.of("g++", "-O2", "-std=c++17", cpp.toString(), "-o", exe.toString()), null, 5);
            if ((int)compileResult.getOrDefault("returncode", 1) != 0) {
                return Map.of("error", "Compilation failed", "stderr", compileResult.get("stderr"));
            }

            // Run
            return runProcess(List.of(exe.toString()), userInput, 5);
        } catch (Exception e) {
